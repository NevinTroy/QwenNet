...
def load_or_initialize_parameters(args, model):
    ...
    is_qwen_model = hasattr(model, 'is_qwen') and model.is_qwen
    
    if args.pretrained_model_path is not None:
        # Initialize with pretrained model.
        from uer.model_loader import load_model
        model = load_model(model, args.pretrained_model_path)
    elif is_qwen_model or use_qwen:
        # Qwen models should always start from pretrained weights
        # If no pretrained path is provided, the model is already loaded from Hugging Face
        print("Using pretrained Qwen weights from Hugging Face")
    else:
...
def main():
    ...
    parser.add_argument("--use_qwen", action="store_true",
                        help="Use Qwen model from Hugging Face instead of UER model.")
    parser.add_argument("--qwen_model_name", type=str, default="Qwen/Qwen2.5-0.5B",
                        help="Hugging Face model name for Qwen (e.g., 'Qwen/Qwen2.5-0.5B').")
